...
function getMessagesObjectFromExpression(nodePath: NodePath<any>): NodePath<any> {
...
export const visitor: VisitNodeFunction<PluginPass & State, t.CallExpression> = function (
  path,
  {
    opts,
    file: {
  ...
  const { overrideIdFn, idInterpolationPattern, removeDefaultMessage, ast, preserveWhitespace } =
    opts as Options;
   ...
   */
  function processMessageObject(messageDescriptor: NodePath<t.ObjectExpression>) {
    assertObjectExpression(messageDescriptor, callee);

    const properties = messageDescriptor.get("properties") as NodePath<t.ObjectProperty>[];
    ...
    storeMessage(descriptor, messageDescriptor, opts as Options, filename || undefined, messages);
