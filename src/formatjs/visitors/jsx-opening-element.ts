...
export const visitor: VisitNodeFunction<PluginPass & State, t.JSXOpeningElement> = function (
  path,
  {
    opts,
    file: {
  ...
  const { removeDefaultMessage, idInterpolationPattern, overrideIdFn, ast, preserveWhitespace } =
    opts as Options;
  ...
  if (!componentNames.find((n: any) => name.isJSXIdentifier({ name: n }))) {
    return;
  }
  ...
  storeMessage(descriptor, path, opts as Options, filename || undefined, messages);
  ...
  for (const attr of attributes) {
    ...
    switch (getMessageDescriptorKey((attr as NodePath<t.JSXAttribute>).get("name"))) {
      case "description":
  ...
  if (defaultMessageAttr) {
    if (removeDefaultMessage) {
      ...
      const valueAttr = defaultMessageAttr.get("value") as NodePath<t.JSXExpressionContainer>;
      valueAttr
        .get("expression")
